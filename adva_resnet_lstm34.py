...
class AWLSTM:
    def __init__(self, data_path, model_path, model_save_path, parameters, steps=1, epochs=50,
                 ...
                 fix_init=0, adv=0, reload=0, resnet=1):
    ...
    def adv_part(self, adv_inputs):
        ...
        if self.att:
            with tf.compat.v1.variable_scope('pre_fc'):
                fc_W = tf.Variable(
                    initial_value=tf.keras.initializers.GlorotUniform()(shape=[self.paras['unit'] * 2, 1]),
                    trainable=True,
                    dtype=tf.float32,
                    name='weights'
                )
                ...
                if self.hinge:
                    pred = tf.nn.bias_add(
                        tf.matmul(adv_inputs, fc_W), fc_b
                    )
                else:
                    pred = tf.nn.sigmoid(
                        tf.nn.bias_add(tf.matmul(self.fea_con, fc_W), fc_b)
                    )
        else:
            # One hidden layer
            pre_fc_layer = tf.keras.layers.Dense(
                units=1,
                activation=None if self.hinge else tf.nn.sigmoid,
                kernel_initializer=tf.keras.initializers.GlorotUniform(),
                name='pre_fc'
            )
            pred = pre_fc_layer(adv_inputs)
        return pred
    ...
    def construct_graph(self):
        ...
        with tf.device(device_name):
            ...
            #self.in_lat = tf.keras.layers.Dense(
                #units=self.fea_dim,
            ...
            #self.outputs = tf.keras.layers.LSTM(
                #units=self.paras['unit'],
                #activation='tanh',  # 或者您可以根据需要选择其他激活函数
                #return_sequences=True,  # 如果需要输出所有时间步的结果，请将其设置为True
            #)(self.in_lat)
            ...
            if self.resnet:
                self.resnet_input_shape = tf.expand_dims(self.pv_var, axis=-1)
                # resnet_output_dim = 1024  # The dimension of ResNet output
                # Pass the data through the ResNet model for feature extraction
                resnet_model = ResNetTypeI(self.resnet_input_shape, layer_params=[3, 4, 6, 3])  # Use ResNet18
                resnet_output = resnet_model(self.resnet_input_shape, training=True)
                # self.resnet_features = tf.compat.v1.layers.dense(
                #     inputs=resnet_output, units=self.fea_dim, activation=tf.nn.tanh)
                self.resnet_features = tf.keras.layers.Dense(
                        units=4, activation=tf.nn.tanh)(resnet_output)  # # 与 self.outputs 的第二个维度匹配 这里的 resnet_output 应该是 ResNet 模型的输出
#                   # 扩展 self.resnet_features 的形状以匹配 self.outputs 的形状

                self.resnet_features_expanded = tf.tile(tf.expand_dims(self.resnet_features, axis=1), multiples=[1, self.outputs.get_shape().as_list()[1], 1])
            # 获取 self.outputs 和 self.resnet_features 的形状
        
        # 扩展 self.resnet_features 的形状以匹配 self.outputs 的形状
                # self.resnet_features_expanded = tf.tile(tf.expand_dims(self.resnet_features, axis=-1), multiples=[1, 1, 4])


                self.resnet_features = self.resnet_features_expanded
                outputs_shape = self.outputs.get_shape().as_list()
                resnet_features_shape = self.resnet_features.get_shape().as_list()

                print(f"outputs_shape: {outputs_shape}")
                print(f"resshape: {resnet_features_shape}")
            # 检查最后一个维度是否相同
                if outputs_shape[-1] != resnet_features_shape[-1]:
                    raise ValueError("Last dimensions of outputs and resnet_features must match.")
# 

                print("Shape of self.output:", tf.shape(self.outputs))
                # self.resnet_features = tf.tile(tf.expand_dims(self.resnet_features, axis=1), multiples=[1, self.outputs.get_shape().as_list()[1], 1])

                # 检查 self.resnet_features 的形状
                resnet_features_shape = tf.shape(self.resnet_features)
                print("Shape of self.resnet_features:", resnet_features_shape)

                # 先确保输出的维度匹配
                assert self.outputs.get_shape().as_list()[-2:] == self.resnet_features.get_shape().as_list()[-2:], "Output dimensions must match."

                # 将两个张量连接起来，axis=-1 表示在最后一个维度上进行连接
                # self.combined_features = tf.concat([self.outputs, self.resnet_features], axis=-1)
                self.combined_features = tf.multiply(self.outputs, self.resnet_features)

                print("Shape of self.combined_features:", self.combined_features.get_shape().as_list())

            else:
                self.combined_features = self.outputs
                print("Shape of self.combined_features:", self.combined_features.get_shape().as_list())
            ...
            if self.att:
                with tf.compat.v1.variable_scope('lstm_att') as scope:
                    ...
                    self.a_laten = tf.tanh(
                        tf.tensordot(self.combined_features, self.av_W,
                                     axes=1) + self.av_b)
                    ...
                    self.a_con = tf.reduce_sum(
                        self.combined_features * tf.expand_dims(self.a_alphas, -1), 1)
                    self.fea_con = tf.concat(
                        [self.combined_features[:, -1, :], self.a_con],
                        axis=1)
                ...
                with tf.compat.v1.variable_scope('lstm_att') as scope:
                    print('adversarial scope')
                    # training loss
                    self.pred = self.adv_part(self.combined_features[:, -1, :])
                    if self.hinge:
                        self.loss = tf.compat.v1.losses.hinge_loss(self.gt_var, self.pred)
                    else:
                        self.loss = tf.compat.v1.losses.log_loss(self.gt_var, self.pred)
                    ...
                    if self.adv_train:
                        print('gradient noise')
                        self.delta_adv = tf.gradients(self.loss, [self.combined_features[:, -1, :]])[0]
                        tf.stop_gradient(self.delta_adv)
                        self.delta_adv = tf.nn.l2_normalize(self.delta_adv,
                                                            axis=1)
                        self.adv_pv_var = self.combined_features[:, -1, :] + \
                                          self.paras['eps'] * self.delta_adv
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rn','--resnet',type=int, default=0,
